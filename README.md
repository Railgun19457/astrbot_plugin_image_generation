...
`/生图` 会提取“比例、宽高比、画幅、分辨率”附近明确写出的参数，兼容中文冒号、空格和 `k/K` 大小写，也支持末尾连续、独立的比例与分辨率简写（顺序不限）。数量仍放在最后，例如 `16:9 2K 3` 表示比例 `16:9`、分辨率 `2K`、生成数量 `3`；`16 : 9` 的末尾 `9` 不会被当成数量。正文中没有关键词的孤立数字和尺寸不参与提取，例如“猫在 16:9 屏幕前”。
