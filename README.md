...
| 由 LLM 工具调用处理命令 | 关闭 | 开启后，下方勾选的命令不再由插件直接提交生图，而是交给当前会话的聊天模型，结合上下文与人设再调用 `generate_image` 等工具。需同时启用「生图工具」，否则仍按原命令执行。 |
| 由 LLM 接管的命令 | 生图 | 仅在上一开关开启时生效。首期仅支持「生图」。 |

### 命令交由 LLM 处理

默认情况下，`/生图` 由插件在本地解析提示词并直接提交生图任务。用户写「画出你自己的形象」这类依赖 Bot 人设或群聊上下文的短句时，提示词会几乎原样进入画图接口，效果往往不理想。

开启 **「由 LLM 工具调用处理命令」** 并勾选 **「生图」** 后：

- 有内容的 `/生图 ...` 会交给**当前会话**的聊天模型（带上对话历史与人设），由模型理解意图并调用本插件的 `generate_image` 工具写出具体 prompt。
- 之后的任务队列、审核、额度与完成后的 AI 唤醒发图，与用户自然语言触发生图工具时相同（任务来源为 LLM 工具，而不是指令直提）。
- 命令末尾的单独数字仍会解析为生成数量，并写入交给模型的说明，要求其传入 `image_count`。
- 当前消息或引用消息中的图片会作为 `image_urls` 传给会话模型，便于「按这张图改」类需求。

以下情况**不会**走 LLM 接管，行为与关闭开关时一致：

- 开关关闭，或未在「由 LLM 接管的命令」中勾选「生图」。
- 未启用「生图工具」。
- 当前会话没有可用的聊天模型（无法 `request_llm`）——此时回退为插件直接执行 `/生图`。
- 空的 `/生图`（仅显示本地帮助）、`/生图任务`、`/生图取消`、`/生图模型`、`/预设` 等其它命令。

**与自然语言出图的关系：** 接管后的 `/生图` 在效果上接近用户直接说「帮我画…」并让模型调工具，只是入口仍是斜杠命令。若你只希望模糊需求走 LLM、明确提示词仍要原样速画，保持该开关关闭即可。
